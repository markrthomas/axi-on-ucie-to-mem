...
// define the SAME names — compile ONE or the other, never both.
//
// EDA Playground / VCS setup:
//   * Testbench pane : this file (eda/vcs_uvm/testbench.sv).
//   * Design pane    : eda/vcs_uvm/design.sv (rtl/*.sv concatenated).
//   * Testbench + Libraries : SystemVerilog / UVM 1.2.
//   * Tools & Simulators    : Synopsys VCS.
//   * Run options (optional): +UVM_TESTNAME=axi_write_read_test
//                             (or axi_random_test / axi_walking_test).
//     With no run option it defaults to axi_write_read_test.
//   * Waves: add +define+DUMP (compile) to emit axi_ucie_tb_top.vcd for EPWave.
//
// Equivalent VCS command line (what EDA Playground runs under the hood):
//   vcs -full64 -sverilog -ntb_opts uvm-1.2 -timescale=1ns/1ps \
//       design.sv testbench.sv -o simv && ./simv +UVM_TESTNAME=axi_write_read_test
// -----------------------------------------------------------------------------
`timescale 1ns/1ps
